...
/// Simple buffer pool to avoid allocations for UDS
#[allow(dead_code)]
struct UdsBufferPool {
...
thread_local! {
    static UDS_BUFFER_POOL: UdsBufferPool = UdsBufferPool::new(8);
}
...
impl UnixDomainSocketTransport {
    ...
    async fn read_message(stream: &mut UnixStream) -> Result<Message> {
        ...
        let mut message_data = UDS_BUFFER_POOL.with(|pool| pool.get_buffer(message_len));
