...
impl UnixDomainSocketTransport {
    ...
    async fn read_message(stream: &mut UnixStream) -> Result<(Message, u64)> {
    ...
    async fn write_message(stream: &mut UnixStream, message: &Message) -> Result<(), IpcError> {
        ...
        if let Some(ref mut stream) = self.stream {
            ...
            match Self::write_message(stream, message).await {
                Ok(()) => {
                    ...
                    if elapsed > std::time::Duration::from_millis(5) {
                        ...
                        if !self.has_warned_backpressure {
                            warn!(
                                "UDS backpressure detected (send took {:?}). \
                ...
                Err(IpcError::BackpressureTimeout) => {
                    if !self.has_warned_backpressure {
                        warn!(
                            "UDS send timed out due to backpressure. \
...
mod tests {
    ...
    async fn test_uds_backpressure() {
        ...
        let large_payload = vec![0; 1024]; // A reasonably large payload.
        ...
        assert!(
            ...
            "A backpressure timeout was not detected when the UDS buffer was full"
        );
    ...
    /// Test Default impl and trait accessors for UDS transport.
    #[test]
    fn test_uds_default_and_trait_accessors() {
        let transport = UnixDomainSocketTransport::default();
        assert_eq!(transport.name(), "Unix Domain Socket");
    ...
    #[tokio::test]
    async fn test_uds_send_when_not_connected() {
        ...
        assert!(
            result.is_err(),
            "send on unconnected UDS transport should fail"
        );
    }

    /// Test that receive fails when the transport is not connected.
    #[tokio::test]
    async fn test_uds_receive_when_not_connected() {
        let mut transport = UnixDomainSocketTransport::new();
        let result = transport.receive().await;
        assert!(
            result.is_err(),
            "receive on unconnected UDS transport should fail"
        );
    }

    /// Test send_to_connection and close_connection on a multi-server.
    #[tokio::test]
    async fn test_uds_send_to_and_close_connection() {
        let socket_path = get_temp_socket_path("test_uds_conn.sock");
        let config = TransportConfig {
            socket_path: socket_path.clone(),
            ..Default::default()
        };
