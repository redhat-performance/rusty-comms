...
mod tests {
    ...
    /// Exercises `write_data_polling()` directly to cover the
    /// polling-based write fallback path. This function is only
    /// called at runtime when pthread condvar primitives are
    /// detected as broken, so it needs a direct unit test.
    ///
    ...
    fn test_write_data_polling_and_read() {
        ...
        // Initialize using the proper constructor to set up
        // pthread primitives correctly.
        let rb_init = SharedMemoryRingBuffer::new(capacity);
        unsafe {
            std::ptr::write(ptr as *mut SharedMemoryRingBuffer, rb_init);
        }
        let rb = unsafe { &*(ptr as *const SharedMemoryRingBuffer) };

        // Test 1: write_data_polling without timestamp update
        ...
        // Read it back using the non-blocking read path
        let read_back = rb.read_data().unwrap();
        assert_eq!(
            read_back,
            vec![0xAAu8; 16],
            "Polling write data should match read"
        );

        // Test 2: write_data_polling with timestamp update
        let mut payload2 = vec![0u8; 20];
        // Put a known pattern in the "timestamp" region
        // (bytes 4..12)
        payload2[4..12].copy_from_slice(&[0xFF; 8]);
        let ts_range = Some(4..12usize);

        unsafe {
            rb.write_data_polling(&mut payload2, ts_range).unwrap();
        }

        // The timestamp region should have been updated to
        // a real monotonic time (no longer 0xFF bytes).
        let ts_val = u64::from_le_bytes(payload2[4..12].try_into().unwrap());
        assert_ne!(
            ts_val,
            u64::from_le_bytes([0xFF; 8]),
            "Timestamp should have been updated in-place"
        );
        assert!(ts_val > 0, "Timestamp should be non-zero");

        // Read back and verify the data
        let read_back2 = rb.read_data().unwrap();
        assert_eq!(read_back2.len(), 20);

        // Test 3: shutdown causes error when buffer is full.
        // Fill the remaining buffer so the polling loop is
        // entered and checks the shutdown flag.
        rb.shutdown.store(false, Ordering::Release);
        loop {
            let mut filler = vec![0xCCu8; 10];
            let result = unsafe { rb.write_data_polling(&mut filler, None) };
            if result.is_err() {
                break; // Buffer full
            }
        }
        // Now set shutdown and try to write
        rb.shutdown.store(true, Ordering::Release);
        let mut payload3 = vec![0xBBu8; 8];
        let result = unsafe { rb.write_data_polling(&mut payload3, None) };
        assert!(result.is_err(), "Should error on shutdown when buffer full");
