...
MSG_SIZE=${2:-1024}
WORKERS=${3:-1}
SOCKET_PATH=${SOCKET_PATH:-"./sockets/ipc_benchmark.sock"}

OUTPUT_DIR="./output"
OUTPUT_FILE=${OUTPUT_FILE:-"${OUTPUT_DIR}/host_client_results.json"}

echo -e "${BLUE}=== UDS Host Client ===${NC}"
...
status=$?
if [ $status -eq 0 ]; then
  echo -e "${GREEN}[SUCCESS]${NC} Host UDS benchmark completed"
  ...
  echo -e "${RED}[ERROR]${NC} Host UDS benchmark failed"
fi
