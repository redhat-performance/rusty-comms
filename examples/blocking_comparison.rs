...
async fn main() -> Result<()> {
    ...
    let async_runner = BenchmarkRunner::new(
        async_config,
    ...
    let blocking_runner = BlockingBenchmarkRunner::new(
        blocking_config,
        IpcMechanism::TcpSocket,
        blocking_args,
    );
    ...
    if let (Some(async_ow), Some(blocking_ow)) = 
        (&async_results.one_way_results, &blocking_results.one_way_results) {
        
        if let (Some(async_lat), Some(blocking_lat)) = 
            (&async_ow.latency, &blocking_ow.latency) {
            ...
            print_comparison_row(
                "Mean", 
                async_lat.mean_ns as f64, 
                blocking_lat.mean_ns as f64
            );
            print_comparison_row(
                "Median", 
                async_lat.median_ns as f64, 
                blocking_lat.median_ns as f64
            );
            print_comparison_row(
                "Min", 
                async_lat.min_ns as f64, 
                blocking_lat.min_ns as f64
            );
            print_comparison_row(
                "Max", 
                async_lat.max_ns as f64, 
                blocking_lat.max_ns as f64
            );
        ...
        let throughput_diff = ((blocking_ow.throughput.messages_per_second 
            - async_ow.throughput.messages_per_second) 
            / async_ow.throughput.messages_per_second) * 100.0;
        println!("  Difference: {:.1}%\n", throughput_diff);
    }
    
    // Round-Trip Comparison
    if let (Some(async_rt), Some(blocking_rt)) = 
        (&async_results.round_trip_results, &blocking_results.round_trip_results) {
        
        if let (Some(async_lat), Some(blocking_lat)) = 
            (&async_rt.latency, &blocking_rt.latency) {
        ...
        let throughput_diff = ((blocking_rt.throughput.messages_per_second 
            - async_rt.throughput.messages_per_second) 
            / async_rt.throughput.messages_per_second) * 100.0;
        println!("  Difference: {:.1}%\n", throughput_diff);
    }
    ...
    if let (Some(async_ow), Some(blocking_ow)) = 
        ...
        if let (Some(async_lat), Some(blocking_lat)) = 
            (&async_ow.latency, &blocking_ow.latency) {
            let latency_diff = ((blocking_lat.mean_ns as f64 - async_lat.mean_ns as f64) 
                / async_lat.mean_ns as f64) * 100.0;
            
            if latency_diff.abs() < 5.0 {
                println!("- Latencies are similar (within 5%)");
            } else if latency_diff < 0.0 {
                println!("- Blocking mode has {:.1}% lower latency", latency_diff.abs());
            } else {
