...
class DashboardApp:
    ...
    def __init__(self, data_store: Dict, initial_dir: str = "."):
        ...
        self.app = dash.Dash(__name__, title="rusty-comms")
        
        # Load external CSS file
        css_file_path = os.path.join(
            os.path.dirname(__file__), "dashboard_styles.css"
        )
    ...
    def apply_dark_theme(self, fig):
        """Apply enhanced dark theme matching panel backgrounds with neon
        ...
        fig.update_layout(
            ...
            # Hover styling
            hoverlabel=dict(
                bgcolor="#21262d", bordercolor="#666666", font_color="#ffffff"
            ),
        )
    ...
    def is_empty_state(self) -> bool:
        """Check if the current data store is in empty state."""
        summary_df = self.data_store.get("summary_data", pd.DataFrame())
        streaming_df = self.data_store.get("streaming_data", pd.DataFrame())
        return summary_df.empty and streaming_df.empty

    def get_data_status(self) -> Dict[str, Any]:
        """Get detailed data status from data processor."""
        from data_processor import BenchmarkDataProcessor
        from pathlib import Path
        
        # Create a temporary processor to get status
        temp_processor = BenchmarkDataProcessor(Path(self.initial_dir))
        temp_processor.data_store = self.data_store
        return temp_processor.get_data_status()

    def validate_dashboard_compatibility(self) -> Dict[str, List[str]]:
        """Validate dashboard compatibility and get suggestions."""
        from data_processor import BenchmarkDataProcessor
        from pathlib import Path
        
        # Create a temporary processor to validate
        temp_processor = BenchmarkDataProcessor(Path(self.initial_dir))
        temp_processor.data_store = self.data_store
        return temp_processor.validate_dashboard_compatibility()

    def create_data_status_alert(self):
        """Create status alert showing what data is available/missing."""
        status = self.get_data_status()
        validation = self.validate_dashboard_compatibility()
        
        if status['completely_empty']:
            return self.create_empty_state_message()
        
        if status['missing_streaming']:
            return html.Div([
                html.Div([
                    html.H4("Partial Data Available", className="text-warning mb-3"),
                    html.P([
                        "Summary data found (",
                        html.Strong(f"{status['summary_files']} files"),
                        "), but streaming data missing."
                    ], className="mb-3"),
                    
                    html.Div([
                        html.H5("Missing Features:", className="mb-2"),
                        html.Ul([
                            html.Li(feature) for feature in validation['missing_features']
                        ], className="mb-3")
                    ]),
                    
                    html.Div([
                        html.H5("Required ipc-benchmark Parameters:", className="mb-2"),
                        html.Pre([
                            html.Code("--streaming-output-json", className="text-info")
                        ], className="bg-dark p-2 rounded mb-3"),
                        html.P("Add this parameter to enable Time Series analysis.", className="text-muted")
                    ])
            ...
            return html.Div([
                html.Div([
                    ...
                    html.P([
                        ...
                        html.Strong(f"{status['streaming_files']} files"),
                        "), but summary data missing."
                    ], className="mb-3"),
                    ...
                    html.Div([
                        html.H5("Required ipc-benchmark Parameters:", className="mb-2"),
                        html.Pre([
                            html.Code("-o <directory>", className="text-info")
                        ], className="bg-dark p-2 rounded mb-3"),
                        html.P("Add this parameter to enable Summary analysis.", className="text-muted")
                    ])
                    
                ], className="text-center py-4")
            ], className="container mt-3")
        
        # If we get here, we have full data - return None to use normal rendering
        return None

    def create_help_modal(self):
        """Create help modal with parameter guidance."""
        return html.Div([
            html.Div([
                html.H4("Dashboard Data Requirements", className="text-primary mb-3"),
                html.Hr(),
                
                html.H5("Required ipc-benchmark Parameters:", className="mb-3"),
                html.Div([
                    html.P("For full dashboard functionality, use both parameters:", className="mb-2"),
                    html.Pre([
                        html.Code([
                            "./ipc-benchmark --mechanism SharedMemory --message-size 1024 \\", html.Br(),
                            "                 -o ./dashboard_data/ \\", html.Br(),
                            "                 --streaming-output-json \\", html.Br(),
                            "                 --duration 30s"
                        ], className="text-info")
                    ], className="bg-dark p-3 rounded mb-3"),
                ]),
                
                html.H5("File Output Expectations:", className="mb-3"),
                html.Pre([
                    html.Code([
                        "dashboard_data/", html.Br(),
                        "├── sharedmemory_1024_summary.json     # Summary Analysis", html.Br(),
                        "└── sharedmemory_1024_streaming.json   # Time Series Analysis"
                    ], className="text-success")
                ], className="bg-dark p-3 rounded mb-3"),
                
                html.H5("Common Issues:", className="mb-3"),
                html.Ul([
                    html.Li([html.Strong("No Data Available:"), " Missing --streaming-output-json parameter"]),
                    html.Li([html.Strong("Only Summary Data:"), " Missing -o parameter"]),
                    html.Li([html.Strong("Empty Dashboard:"), " No benchmark files found in directory"])
                ], className="mb-3"),
                
                html.Hr(),
                html.P([
                    "For complete documentation, see ",
                    html.A("README.md", href="../../README.md#dashboard-integration", target="_blank", className="text-primary")
                ], className="text-muted text-center")
            ], className="bg-secondary p-4 rounded", style={"max-width": "600px", "margin": "0 auto"})
        ], id="help-modal", style={"display": "none", "position": "fixed", "top": "10%", "left": "0", "right": "0", "z-index": "9999", "background": "rgba(0,0,0,0.8)"})

    def create_help_button(self):
        """Create help button to show modal."""
        return html.Button([
            html.I(className="fas fa-question-circle", style={"margin-right": "5px"}),
            "Help"
        ], id="help-button", className="btn btn-outline-info btn-sm")

    def create_empty_state_message(self):
        """Create UI component for empty state."""
        return html.Div([
            html.Div([
                html.Div([
                    html.I(className="fas fa-folder-open", 
                           style={'font-size': '4rem', 'color': '#6c757d', 
                                 'margin-bottom': '1rem'}),
                    html.H3("No Benchmark Data Found", 
                           className="text-muted mb-3"),
                    html.P([
                        "Place your JSON result files in the selected directory and click ",
                    ...
                    html.Div([
                        html.H5("Expected File Types:", 
                               className="alert-heading mb-3"),
                        html.Ul([
                            html.Li("Summary JSON files (*_summary.json)"),
                            html.Li("Streaming JSON files (*_streaming.json)"), 
                            html.Li("Streaming CSV files (*_streaming.csv)")
                        ], className="mb-0")
                    ], className="alert alert-info text-start"),
                    
                    html.P([
                        "📂 Current directory: ",
                        html.Code(self.initial_dir, 
                                 className="text-primary")
                    ], className="mt-3 text-muted")

                ], className="text-center py-5")
            ], style={'width': '66.666667%', 'margin': '0 auto'})
    ...
    def reload_data(self, directory_path: str) -> tuple:
        ...
        try:
            ...
            if not dir_path.exists():
                return (
                    False,
                    f"Error: Directory '{directory_path}' does not exist.",
                )
            ...
            logger.info("Reloading data from directory: %s", directory_path)
            ...
            # Update the data store regardless of whether data was found
            self.data_store.update(new_data_store)
            
            # Create appropriate message
            if summary_count == 0 and streaming_count == 0:
                success_msg = (
                    f"📁 Directory '{directory_path}' is ready for data. "
                    f"Add benchmark result files and refresh again to load them."
                )
                logger.info("Directory loaded (empty): %s", directory_path)
            else:
                success_msg = (
                    f"Success: Loaded {summary_count} summary records and "
                    f"{streaming_count:,} streaming records from '{directory_path}'."
                )
    ...
    def get_filter_options(self):
        ...
        # Handle empty state
        if summary_df.empty and streaming_df.empty:
            logger.info("Filter options: Empty state - no data available")
            return {
                "mechanisms": [],
                "message_sizes": []
            }
        ...
        if not summary_df.empty:
            ...
            valid_message_sizes = [
                x for x in summary_df["message_size"].unique() if x > 0
            ]
        ...
        if not streaming_df.empty:
            ...
            # Filter out 0 and invalid message sizes
            valid_message_sizes = [
                x for x in streaming_df["message_size"].unique() if x > 0
            ]
            message_sizes.update(valid_message_sizes)
        ...
        logger.info("Filter options: %s", result)
        return result
    ...
    def setup_layout(self):
        ...
        self.app.layout = html.Div(
                ...
                # Sidebar Controls
                    ...
                    # Analysis Control Section - MOVED TO TOP
                                html.Div(
                                    [
                                        ...
                                        html.P(
                                            "Run analysis with current filters to generate charts and insights.",
                                            style={
                                                "color": "#6b7280",
                                                "font-size": "0.9rem",
                                                "margin-bottom": "15px",
                    ...
                    # Data Directory Controls with File Browser
                        ...
                        html.H3("Data Directory"),
                                        html.P(
                                            "Browse and select a directory containing benchmark data files.",
                                            style={
                                                "color": "#6b7280",
                                                "font-size": "0.9rem",
                                                "margin-bottom": "10px",
                        ...
                        html.H3("Filters"),
                                        html.P(
                                            "Select items to filter data. All options are selected by default.",
                                            style={
                                                "color": "#6b7280",
                                                "font-size": "0.9rem",
                                                "margin-bottom": "15px",
                        ...
                        dcc.Checklist(
                                            ...
                                            options=[
                                                {"label": m, "value": m}
                                                for m in filter_options[
                                                    "mechanisms"
                                                ]
                                            ],
                                            value=filter_options[
                                                "mechanisms"
                                            ],  # Start with all mechanisms selected
                        ...
                        dcc.Checklist(
                                            ...
                                            options=[
                                                {"label": f"{s}B", "value": s}
                                                for s in filter_options[
                                                    "message_sizes"
                                                ]
                                            ],
                                            value=filter_options[
                                                "message_sizes"
                                            ],  # Start with all message sizes selected
                                            className="filter-checklist",
                                        ),
                                    ],
                                    className="filter-section",
            ...
            # File Browser Modal
                ...
                dcc.Store(
                    id="selected-directory-store", data=self.initial_dir
                ),
                # Cache clear indicator state management
                dcc.Store(
                    id="cache-clear-state", 
                    data={"visible": False, "timestamp": 0}
                ),
                dcc.Interval(
                    id="cache-clear-timer",
                    interval=3000,  # 3 seconds
                    max_intervals=1,
                    disabled=True
                ),
                
                # Help Modal for Quick Presets
                html.Div([
                    html.Div([
                        # Header with close button
                        html.Div([
                            html.H4("Quick Presets Guide", className="text-primary mb-3"),
                            html.Button("×", 
                                       id="preset-help-close", 
                                       style={
                                           "position": "absolute", 
                                           "top": "10px", 
                                           "right": "15px", 
                        ...
                        html.P("Click any preset button to instantly configure your Time Series analysis with optimized settings:", 
                              className="mb-4 text-light", style={"font-size": "16px"}),
                        
                        # Preset descriptions
                        html.Div([
                            html.Div([
                                html.H5("Performance Analysis", className="text-warning mb-2"),
                                html.P("Optimized for identifying performance characteristics and bottlenecks. Focuses on mean latencies with rolling averages to highlight trends and performance patterns across different IPC mechanisms.", 
                                      className="mb-3 text-light", style={"font-size": "14px", "line-height": "1.5"}),
                            ], className="mb-4"),
                            
                            html.Div([
                                html.H5("Detailed Inspection", className="text-info mb-2"),
                                html.P("Comprehensive view with multiple latency metrics and percentile overlays. Enables deep-dive analysis with maximum observability into latency distributions, outliers, and statistical behavior.", 
                                      className="mb-3 text-light", style={"font-size": "14px", "line-height": "1.5"}),
                            ], className="mb-4"),
                            
                            html.Div([
                                html.H5("Statistical Overview", className="text-success mb-2"),
                                html.P("Faceted view organized by message size with rolling percentiles and statistical overlays. Perfect for understanding how performance scales with message size and comparing statistical distributions.", 
                                      className="mb-3 text-light", style={"font-size": "14px", "line-height": "1.5"}),
                            ], className="mb-4"),
                            
                            html.Div([
                                html.H5("Outlier Detection", className="text-danger mb-2"),
                                html.P("Specialized configuration for identifying anomalies and performance spikes. Uses separate charts with enhanced visibility for detecting unusual latency patterns and system irregularities.", 
                                      className="mb-3 text-light", style={"font-size": "14px", "line-height": "1.5"}),
                            ], className="mb-4"),
                            
                            html.Div([
                                html.H5("Reset to Default", className="text-secondary mb-2"),
                                html.P("Restores all Time Series controls to their initial default settings while preserving the statistics panel state. Use this to start fresh or undo preset configurations.", 
                                      className="mb-3 text-light", style={"font-size": "14px", "line-height": "1.5"}),
                            ], className="mb-4"),
                        ]),
                        ...
                        html.Div([
                            html.I(className="fas fa-lightbulb", style={"margin-right": "8px", "color": "#ffc107"}),
                            html.Strong("Pro Tip: ", className="text-warning"),
                            html.Span("Each preset is a starting point - you can mix and match settings after applying any preset to create your ideal analysis view.", className="text-light")
                        ], className="small mb-0", style={"background": "rgba(255, 193, 7, 0.1)", "padding": "12px", "border-radius": "6px", "border-left": "4px solid #ffc107"})
                ...
                ], id="preset-help-modal", 
                   style={"display": "none", "position": "fixed", "top": "5%", "left": "0", "right": "0", 
                          "z-index": "9999", "background": "rgba(0,0,0,0.9)", "padding": "20px"}),
            ],
            style={
                "background": "#0d1117",
                "minHeight": "100vh",
    ...
    def setup_callbacks(self):
        ...
        @self.app.callback(
            [
                ...
                Output("cache-clear-timer", "disabled"),
                Output("cache-clear-state", "data")
            ],
            ...
            if n_clicks > 0:
                ...
                cache_div = html.Div(
                    [
                        html.Span(
                            "Cache cleared",
                            style={"color": "#2da44e", "font-weight": "bold"},
                        )
                    ],
                    className="cache-indicator",  # Add animation class
                    style={
                        "padding": "8px",
                        "background": "linear-gradient(135deg, #0d4d21 0%, #0a3b1c 100%)",
                        "border-radius": "4px",
                ...
                return (
                    cache_div, 
                    False,  # Enable timer
                    {"visible": True, "timestamp": time.time()}
                )
            return (
                html.Div(), 
        ...
        # Auto-fade timer callback for cache cleared indicator
        @self.app.callback(
            [
                Output("analysis-status", "children", allow_duplicate=True),
                Output("cache-clear-timer", "disabled", allow_duplicate=True),
                Output("cache-clear-state", "data", allow_duplicate=True)
            ],
            [Input("cache-clear-timer", "n_intervals")],
            [State("cache-clear-state", "data")],
            prevent_initial_call=True,
        )
        def auto_fade_cache_indicator(n_intervals, cache_state):
            # Only proceed if cache indicator is currently visible
            if not cache_state or not cache_state.get("visible", False):
                return dash.no_update, dash.no_update, dash.no_update
            
            # Timer fired - fade out the indicator
            return (
                html.Div(), 
                True,  # Disable timer 
                {"visible": False, "timestamp": time.time()}
            )

        # Main content callback - now handles initial load AND button clicks
        @self.app.callback(
            [
        ...
        @safe_computation(default_return=(html.Div("Error loading content"), dash.no_update, dash.no_update))
        def render_tab_content(
            n_clicks, active_tab, mechanisms, message_sizes, cache_state
        ):
            # Handle cache clearing on Run Analysis
            cache_output = dash.no_update
            cache_state_output = dash.no_update
            
            if (n_clicks and n_clicks > 0 and 
                cache_state and cache_state.get("visible", False)):
                # Clear cache indicator immediately when Run Analysis clicked
                cache_output = html.Div()
                cache_state_output = {"visible": False, "timestamp": time.time()}
            
            # Check for empty state first - show immediately regardless of clicks
            if self.is_empty_state():
                return (
                    self.create_empty_state_message(), 
                    cache_output, 
                    cache_state_output
                )
                
            # Handle initial load (n_clicks is None) or no clicks yet
                        ...
                        [
                            html.Div(
                                [
                                    ...
                                    html.P(
                                        (
                                            "Select your filters and click the 'Run Analysis' button to "
                                    ...
                                    html.Ul(
                                        [
                                            html.Li(
                                                "Choose mechanisms to analyze (PMQ, SHM, UDS)"
                                            ),
                                            html.Li(
                                                "Select message sizes for comparison"
                                            ),
                                            html.Li(
            ...
            try:
                if active_tab == "summary-tab":
                    logger.info(
                        f"Summary tab analysis triggered with filters: mechanisms={mechanisms}, message_sizes={message_sizes}"
                    )
                    return (
                        self.render_summary_tab_cached(mechanisms, message_sizes),
                        cache_output,
                        cache_state_output
                    )
                elif active_tab == "timeseries-tab":
                    # Use callback context to determine what triggered this callback
                    ctx = dash.callback_context
                    triggered_by_analysis_button = False
                    
                    if ctx.triggered:
                        trigger = ctx.triggered[0]['prop_id']
                        triggered_by_analysis_button = trigger.startswith('run-analysis-button')
                    
                    if triggered_by_analysis_button:
                        logger.info(
                            (
                                f"Time series tab analysis triggered with filters: "
                                f"mechanisms={mechanisms}, message_sizes={message_sizes}"
                            )
                        )
                        # For Time Series: Return full tab on first click, then let 
                        # results-only callback handle subsequent updates
                        if not hasattr(self, '_timeseries_tab_initialized'):
                            # First time Time Series analysis is run - show full tab
                            self._timeseries_tab_initialized = True
                            return (
                                self.render_timeseries_tab_cached(mechanisms, message_sizes),
                                cache_output,
                                cache_state_output
                            )
                        else:
                            # Subsequent analysis clicks - let results-only callback handle updates
                            # Main callback doesn't change tab content
                            return (
                                dash.no_update,
                                cache_output,
                                cache_state_output
                        ...
                        # Just switching to the tab - always show tab content
                        logger.info("Switching to Time Series tab")
                        return (
                            self.render_timeseries_tab_cached(mechanisms, message_sizes),
                            cache_output,
                            cache_state_output
        ...
        def update_analysis_button_status(n_clicks, current_style):
            if n_clicks > 0:
                # Reset to normal state (this will be called after analysis
                # completes)
                reset_style = (
                    current_style.copy()
                    if current_style
                ...
                reset_style["background"] = (
                    "linear-gradient(135deg, #1f6feb 0%, #0969da 100%)"  # Grafana blue
                )
                return "Run Analysis", reset_style

            return "Run Analysis", current_style

        # Time series charts update callback - only responds to Run Analysis
        # button and tab changes
        @self.app.callback(
        ...
        @safe_computation(
            default_return=html.Div(
                "Click 'Run Analysis' to generate time series charts"
            )
        )
        def update_timeseries_charts(
            ...
            # Only generate charts if Run Analysis button clicked and we're on
            # time series tab
                    ...
                    [
                        html.Div(
                            [
                                ...
                                html.P(
                                    (
                                        "Configure your analysis settings above and click 'Run Analysis' "
                                        "to generate interactive time series charts."
                                    ),
                                    style={
                                        "color": "#6b7280",
                                        "font-size": "1.1rem",
                                ...
                                html.Div(
                                    [
                                        ...
                                        html.Ul(
                                            [
                                                html.Li(
                                                    "Interactive zoom and pan across all charts"
                                                ),
                                                html.Li(
                                                    "Multiple statistical overlays and anomaly detection"
                                                ),
                                                html.Li(
                                                    "Flexible layout options and sampling strategies"
                                                ),
                                                html.Li(
                                                    "Performance zones and percentile indicators"
                                                ),
                                                html.Li(
                                                    "Real-time statistics and distribution analysis"
                                                ),
                                            ],
                                            style={
                                                "color": "#f0f6fc",
            ...
            display_points = (
                display_points if display_points is not None else 10000
            )
            chart_layout = (
                chart_layout if chart_layout is not None else "separate"
            )
            view_options = (
                view_options if view_options is not None else ["sync_zoom"]
            )
            latency_types = (
                latency_types if latency_types is not None else ["one_way"]
            )
            statistical_overlays = (
                statistical_overlays
                if statistical_overlays is not None
                else ["percentile_bands"]
            )
        ...
        def update_stats_panel(
            ...
            # Only show stats panel if Run Analysis was clicked and we're on
            # time series tab
            if not (n_clicks > 0 and active_tab == "timeseries-tab"):
                return html.Div()
            ...
            if not latency_types:
                latency_types = [
                    "one_way"
                ]  # Default to one-way if nothing selected
            
            # Generate statistics overview
            streaming_df = self.filter_data(
                self.data_store["streaming_data"], mechanisms, message_sizes
            )
            ...
            for mechanism in sorted(streaming_df["mechanism"].unique()):
                mechanism_data = streaming_df[
                    streaming_df["mechanism"] == mechanism
                ]
                ...
                for latency_type in latency_types:
                    ...
                    if (
                        latency_col not in mechanism_data.columns
                        or mechanism_data[latency_col].isna().all()
                    ):
                        continue
                    
                    stats_data = []
                    for msg_size in sorted(
                        mechanism_data["message_size"].unique()
                    ):
                        subset_data = mechanism_data[
                            mechanism_data["message_size"] == msg_size
                        ]
                        if (
                            len(subset_data) > 0
                            and not subset_data[latency_col].isna().all()
                        ):
                            stats_data.append(
                                {
                    ...
                    if stats_data:
                        ...
                        table = html.Div(
                            [
                                html.H5(
                                    f"{mechanism} - {type_label} Latency",
                                    style={
                                        "margin-bottom": "10px",
                                        "color": "#f0f6fc",
                                        "font-weight": "bold",
        ...
        # New results-only callback for Time Series tab
        @self.app.callback(
            Output("timeseries-results-container", "children"),
            [Input("run-analysis-button", "n_clicks")],
            [
        ...
        @safe_computation(
            default_return=html.Div("Click 'Run Analysis' to generate results")
        )
        def update_timeseries_results_only(
            n_clicks,
            active_tab,
            moving_avg_window,
            display_points,
            ...
            if active_tab != "timeseries-tab" or n_clicks is None or n_clicks == 0:
                return dash.no_update

            # Generate the complete Time Series results (charts + stats)
            return self.render_timeseries_charts_and_stats(
                mechanisms,
                message_sizes,
                moving_avg_window,
                display_points,
        ...
        def toggle_file_browser_modal(
            browse_clicks, close_clicks, cancel_clicks
        ):
            ctx = dash.callback_context
            if not ctx.triggered:
                return {"display": "none"}
                ...
                listing_items.append(
                    html.Div(
                        [
                            ...
                            html.Span(
                                item_info["name"], style={"font-size": "13px"}
                            ),
                    ], 
                    id=item_id,
                    style=item_style,
                ...
                file_summary = html.Div(
                    [
                        ...
                        html.Div(
                            [
                                html.Span(
                                    f"Found {file_count} data file{'s' if file_count != 1 else ''}",
                                    style={
                                        "font-size": "12px",
                                        "color": "#3fb950",
                                        "font-weight": "bold",
            ...
            for i, crumb in enumerate(breadcrumbs):
                if i > 0:
                    breadcrumb_items.append(
                        html.Span(
                            " > ",
                            style={"margin": "0 5px", "color": "#9ca3af"},
                        )
                    )
        ...
        def handle_data_reload(n_clicks, selected_directory):
            ...
            if success:
                ...
                return (
                    ...
                    [
                        {"label": m, "value": m}
                        for m in new_filter_options["mechanisms"]
                    ],
                    new_filter_options[
                        "mechanisms"
                    ],  # Select all mechanisms by default
                    [
                        {"label": str(s), "value": s}
                        for s in new_filter_options["message_sizes"]
                    ],
                    new_filter_options[
                        "message_sizes"
                    ],  # Select all message sizes by default
                )
            else:
                ...
                # Keep existing filter options on error and maintain all
                # selections
                ...
                return (
                    ...
                    [
                        {"label": m, "value": m}
                        for m in current_filter_options["mechanisms"]
                    ],
                    current_filter_options[
                        "mechanisms"
                    ],  # Keep all mechanisms selected
                    [
                        {"label": str(s), "value": s}
                        for s in current_filter_options["message_sizes"]
                    ],
                    current_filter_options[
                        "message_sizes"
                    ],  # Keep all message sizes selected
                )

        # Help Modal Toggle Callback
        @self.app.callback(
            Output("preset-help-modal", "style"),
            [
                Input("preset-help-icon", "n_clicks"),
                Input("preset-help-close", "n_clicks"),
            ],
            prevent_initial_call=True,
        )
        def toggle_preset_help_modal(help_clicks, close_clicks):
            """Toggle the preset help modal visibility."""
            # Base modal styles
            base_style = {
                "position": "fixed", 
                "top": "10%", 
            ...
            if button_id == "preset-help-icon" and help_clicks:
                return {**base_style, "display": "block"}
            elif button_id == "preset-help-close" and close_clicks:
                return {**base_style, "display": "none"}
    ...
    def filter_data(
        self, df: pd.DataFrame, mechanisms: List, message_sizes: List
    ) -> pd.DataFrame:
        ...
        # Apply filters only if selections are made. Empty list = show all
        # data for that dimension
    ...
    def generate_performance_insights(
        ...
        throughput_df: pd.DataFrame = None,
        max_latency_df: pd.DataFrame = None,
    ) -> Dict:
        """
        Generate mechanism + message size specific performance recommendations
        based on lowest jitter and worst-case latency.
        """
        insights = {
            "best_mechanism": None,
            "best_latency": None,
        ...
        if percentile_df.empty:
            insights["recommendations"].append(
                "**NO DATA**: Select mechanisms and message sizes to generate performance insights"
            )
            return insights
        ...
        if one_way_p50.empty or max_latency_df is None or max_latency_df.empty:
            insights["recommendations"].append(
                "**INSUFFICIENT DATA**: Need P50, P95, and max latency data for analysis"
            )
            return insights
        
        one_way_max = max_latency_df[
            max_latency_df["latency_type"] == "One-way"
        ]
        if one_way_max.empty:
            insights["recommendations"].append(
                "**INSUFFICIENT DATA**: Need max latency data for analysis"
            )
            return insights
        
        # Calculate performance scores for each mechanism + message size
        # combination
        performance_scores = []
        ...
        for message_size in sorted(one_way_p50["message_size"].unique()):
            ...
            for mechanism in size_p50["mechanism"].unique():
                p50_data = size_p50[size_p50["mechanism"] == mechanism][
                    "latency_us"
                ]
                p95_data = size_p95[size_p95["mechanism"] == mechanism][
                    "latency_us"
                ]
                max_data = size_max[size_max["mechanism"] == mechanism][
                    "max_latency_us"
                ]

                if (
                    not p50_data.empty
                    and not p95_data.empty
                    and not max_data.empty
                ):
                    ...
                    # Combined score: normalize both jitter and max latency,
                    # then combine
                    ...
                    performance_scores.append(
                        {
                            ...
                            "jitter": jitter,
                            "score": max_val
                            + jitter,  # Simple combination: max latency + jitter penalty
                        }
                    )
        
        if not performance_scores:
            insights["recommendations"].append(
                "**NO DATA**: Unable to calculate performance scores"
            )
            return insights
        
        # Generate recommendations by message size
        message_sizes = sorted(
            set(score["message_size"] for score in performance_scores)
        )
        ...
        for message_size in message_sizes:
            size_scores = [
                ...
                if s["message_size"] == message_size
            ]
            ...
            if len(size_scores) == 2:
                ...
                score_improvement = (
                    (worst["score"] - best["score"]) / worst["score"]
                ) * 100
                insights["recommendations"].append(
                    f"**{message_size}B**: {best['mechanism']} excels with "
                    f"{best['jitter']:.1f}μs jitter + {best['max']:.1f}μs max latency "
                    f"({score_improvement:.0f}% better than {worst['mechanism']})"
                )
            else:
                # Multiple mechanisms - show best and mention others
                runner_up = size_scores[1]
                insights["recommendations"].append(
                    f"**{message_size}B**: {best['mechanism']} leads with "
                    f"{best['jitter']:.1f}μs jitter + {best['max']:.1f}μs max latency "
                    f"(beats {runner_up['mechanism']} and {len(size_scores)-2} others)"
                )

        # Overall analysis: find the mechanism that appears most often as
        # winner
        mechanism_wins = {}
        for message_size in message_sizes:
            size_scores = [
        ...
        if mechanism_wins:
            ...
            if win_count == total_sizes:
                insights["recommendations"].insert(
                    0,
                    (
                        f"**CHAMPION**: {overall_winner} dominates across ALL {total_sizes} "
                        f"message sizes with optimal jitter + max latency"
                    ),
                )
                ...
                insights["recommendations"].insert(
                    0,
                    (
                        f"**TOP PERFORMER**: {overall_winner} wins {win_count}/{total_sizes} "
                        f"message sizes with superior jitter + max latency control"
                    ),
                )
            else:
                insights["recommendations"].insert(
                    0,
                    (
                        f"**CONTEXT MATTERS**: No single winner - choose {overall_winner} "
                        f"for {win_count} sizes, others context-dependent"
                    ),
                )
        ...
        extreme_outliers = [
            s
            for s in performance_scores
            if s["max"] > s["p50"] * 20 and s["max"] > 100
        ]
        if extreme_outliers:
            worst_outlier = max(
                extreme_outliers, key=lambda x: x["max"] / x["p50"]
            )
            ratio = worst_outlier["max"] / worst_outlier["p50"]
            insights["recommendations"].insert(
                0,
                (
                    f"**WARNING**: {worst_outlier['mechanism']} at "
                    f"{worst_outlier['message_size']}B has extreme spikes "
                    f"({worst_outlier['max']:.0f}μs max vs "
                    f"{worst_outlier['p50']:.1f}μs typical = {ratio:.0f}x worse)"
                ),
            )
        
        # Throughput analysis for summary stats
        if throughput_df is not None and not throughput_df.empty:
            one_way_throughput = throughput_df[
                throughput_df["type"] == "One-way"
            ]
            if not one_way_throughput.empty:
                best_throughput_row = one_way_throughput.loc[
                    one_way_throughput["msgs_per_sec"].idxmax()
                ]
    ...
    def create_performance_comparison_matrix(
        self, percentile_df: pd.DataFrame, max_latency_df: pd.DataFrame = None
    ) -> tuple:
        ...
        if not percentile_df.empty:
            ...
            if not p50_data.empty:
                # Create pivot table with mechanisms as rows and message sizes
                # as columns
                p50_matrix = p50_data.pivot(
                    index="mechanism",
                    columns="message_size",
                    values="latency_us",
                )

                # Calculate relative performance (percentage compared to best
                ...
                for col in p50_matrix.columns:
                    ...
                    if not col_data.empty:
                        ...
                        p50_matrix[col] = (
                            (col_data - best_value) / best_value * 100
                        ).round(1)
        ...
        if max_latency_df is not None and not max_latency_df.empty:
            ...
            if not max_data.empty:
                # Create pivot table with mechanisms as rows and message sizes
                # as columns
                max_matrix = max_data.pivot(
                    index="mechanism",
                    columns="message_size",
                    values="max_latency_us",
                )
    ...
    def create_summary_cards(
            ...
            [
                html.Div(
                    [
                        ...
                        html.H2(
                            insights.get("best_mechanism", "N/A"),
                            style={"margin": "5px 0", "color": "#065f46"},
                        ),
                        html.P(
                            f"P50: {insights.get('best_latency', 0):.1f}μs",
                            style={
                                "margin": "0",
                                "color": "#8b949e",
                                "font-size": "0.9rem",
                ...
                html.P(
                    (
                        "The mechanism with the best balance of typical performance (P50) "
                        "and consistency. Combines low latency with predictable behavior - "
            ...
            [
                html.Div(
                    [
                        ...
                        html.H2(
                            f"{min_latency:.1f} - {max_latency:.1f}μs",
                            style={"margin": "5px 0", "color": "#1e3a8a"},
                        ),
                        html.P(
                            f"{best_mechanism} (P99: {p99_latency:.1f}μs)",
                            style={
                                "margin": "0",
                                "color": "#8b949e",
                                "font-size": "0.9rem",
                ...
                html.P(
                    (
                        "Shows the latency spread for the best mechanism. Smaller ranges indicate "
                        "more predictable performance. P99 represents near-worst-case latency."
                    ),
                    style={
                        "margin": "8px 0 0 0",
                        "font-size": "0.75rem",
            ...
            [
                html.Div(
                    [
                        ...
                        html.H2(
                            f"{throughput_info.get('msgs_per_sec', 0):,.0f}",
                            style={"margin": "5px 0", "color": "#92400e"},
                        ),
                        html.P(
                            f"msgs/sec ({throughput_info.get('mechanism', 'N/A')})",
                            style={
                                "margin": "0",
                                "color": "#8b949e",
                                "font-size": "0.9rem",
                ...
                html.P(
                    (
                        "Highest message rate achieved across all mechanisms. Important for "
                        "high-volume applications that prioritize throughput over latency."
                    ),
                    style={
                        "margin": "8px 0 0 0",
                        "font-size": "0.75rem",
        ...
        if max_latency_df is not None and not max_latency_df.empty:
            ...
            if not one_way_max.empty:
                ...
                best_max_mechanism = one_way_max.loc[
                    one_way_max["max_latency_us"].idxmin(), "mechanism"
                ]
                ...
                max_latency_card = html.Div(
                    [
                        ...
                        html.P(
                            (
                                "Mechanism with the lowest worst-case latency. Critical for real-time "
                                "systems where maximum response time must be guaranteed."
                            ),
                            style={
                                "margin": "8px 0 0 0",
                                "font-size": "0.75rem",
    ...
    @cached_computation()
    def calculate_percentiles_from_streaming(
        self, mechanisms: List, message_sizes: List
    ):
        """Calculate percentiles dynamically from streaming data."""
        logger.info(
            f"calculate_percentiles_from_streaming: Original data shape: {self.data_store['streaming_data'].shape}"
        )
        streaming_df = self.filter_data(
            self.data_store["streaming_data"], mechanisms, message_sizes
        )
        logger.info(
            f"calculate_percentiles_from_streaming: Filtered data shape: {streaming_df.shape}"
        )
        ...
        for (mechanism, message_size), group in streaming_df.groupby(
            ["mechanism", "message_size"]
        ):
                        ...
                        [
                            {
                                ...
                                "latency_us": np.percentile(
                                    one_way_latencies, 50
                                ),
    ...
    def calculate_max_latencies(self, mechanisms: List, message_sizes: List):
        ...
        logger.info(
            f"calculate_max_latencies: Original data shape: {self.data_store['streaming_data'].shape}"
        )
        streaming_df = self.filter_data(
            self.data_store["streaming_data"], mechanisms, message_sizes
        )
        logger.info(
            f"calculate_max_latencies: Filtered data shape: {streaming_df.shape}"
        )
        
        if streaming_df.empty:
            logger.warning(
                "Filtered streaming data is empty for max latency calculation"
            )
            return pd.DataFrame()
    ...
    def create_pivot_table(
        ...
        data: List[Dict],
        value_col: str,
        index_col: str = "message_size",
        columns_col: str = "mechanism",
        round_digits: int = 2,
    ) -> pd.DataFrame:
        ...
        pivot.columns = [
            str(col) if col != index_col else "Message Size (bytes)"
    ...
    def render_summary_tab(self, mechanisms: List, message_sizes: List):
        ...
        logger.info(
            "render_summary_tab called with: mechanisms=%s, message_sizes=%s", mechanisms, message_sizes
        )

        # Check for empty or partial state first
        data_status_alert = self.create_data_status_alert()
        if data_status_alert is not None:
            return data_status_alert
        ...
        return self._assemble_summary_layout(
            [insights_section, comparison_section, charts_section]
        )
    ...
    def _prepare_summary_data(
        ...
        percentile_df = self.calculate_percentiles_from_streaming(
            mechanisms, message_sizes
        )
        logger.info(
            f"Percentile df shape: {percentile_df.shape}, empty: {percentile_df.empty}"
        )
        ...
        summary_df = self.filter_data(
            self.data_store["summary_data"], mechanisms, message_sizes
        )
        streaming_df = self.filter_data(
            self.data_store["streaming_data"], mechanisms, message_sizes
        )
        ...
        message_size_order = (
            sorted(streaming_df["message_size"].unique())
            if not streaming_df.empty
            else []
        )
    ...
    def _create_no_data_message(self) -> html.Div:
        ...
        return html.Div(
            ...
            html.H3("No Data Available"),
                html.P(
                    "No streaming data available for the selected filters. Try adjusting your filter selections.",
                    style={
                        "color": "#6b7280",
                        "fontSize": "1.1rem",
                        "textAlign": "center",
    ...
    def _calculate_summary_statistics(self, filtered_data: Dict) -> Dict:
        ...
        mechanisms = (
            streaming_df["mechanism"].unique()
            if not streaming_df.empty
            else []
        )
        message_sizes = (
            streaming_df["message_size"].unique()
            if not streaming_df.empty
            else []
        )
        
        # Calculate throughput data
        throughput_df = self._calculate_throughput_data(
            summary_df, streaming_df
        )
        
        # Generate structured latency data for pivot tables
        latency_data = self._generate_latency_data_for_tables(
            streaming_df, summary_df
        )
        
        # Generate structured throughput data for pivot tables  
        throughput_data = self._generate_throughput_data_for_tables(
            throughput_df
        )
        
        # Calculate max latency data for comparison
        max_latency_df = self.calculate_max_latencies(
            list(mechanisms), list(message_sizes)
        )
    ...
    def _calculate_throughput_data(
        self, summary_df: pd.DataFrame, streaming_df: pd.DataFrame
    ) -> pd.DataFrame:
        """Calculate throughput data from summary or streaming data."""
        throughput_data = []
        
        # Use summary data if available, otherwise calculate from streaming
        # data
        if not summary_df.empty:
        ...
        if not throughput_data and not streaming_df.empty:
            logger.info(
                "No summary throughput data found, estimating from streaming data..."
            )
            ...
            for (mechanism, message_size), group in streaming_df.groupby(
                ...
                if len(group) > 10:  # Need sufficient samples
                    ...
                    if "timestamp_ns" in group.columns:
                        time_span_sec = (
                            group["timestamp_ns"].max()
                            - group["timestamp_ns"].min()
                        ) / 1e9
                        if time_span_sec > 0:
        ...
        if not throughput_df.empty:
            throughput_df = throughput_df.dropna(
                subset=["msgs_per_sec", "bytes_per_sec"]
            )
            throughput_df = throughput_df[
                (throughput_df["msgs_per_sec"] > 0)
                & (throughput_df["bytes_per_sec"] > 0)
            ]
        
        return throughput_df
    
    def _generate_latency_data_for_tables(
        self, streaming_df: pd.DataFrame, summary_df: pd.DataFrame
    ) -> List:
        """Generate structured latency data for pivot tables."""
        latency_data = []
        ...
        if not streaming_df.empty:
            ...
            avg_latencies = (
                streaming_df.groupby(["mechanism", "message_size"])
                .agg(
                    {
    ...
    def _generate_throughput_data_for_tables(
                ...
                throughput_data.append(
                    {
                        ...
                        "type": row[
                            "type"
                        ],  # Already contains 'One-way' or 'Round-trip'
    ...
    def _create_latency_tables(self, latency_data: List) -> html.Div:
        ...
        max_oneway_data = latency_df[
            (latency_df["type"] == "Maximum")
            & (latency_df["source"] == "One-way")
        ]
        max_roundtrip_data = latency_df[
            (latency_df["type"] == "Maximum")
            & (latency_df["source"] == "Round-trip")
        ]
        avg_oneway_data = latency_df[
            (latency_df["type"] == "Average")
            & (latency_df["source"] == "One-way")
        ]
        avg_roundtrip_data = latency_df[
            (latency_df["type"] == "Average")
            & (latency_df["source"] == "Round-trip")
        ]
        
        # Create pivot tables
        max_oneway_pivot = self.create_pivot_table(
            max_oneway_data.to_dict("records"), "latency_us"
        )
        max_roundtrip_pivot = self.create_pivot_table(
            max_roundtrip_data.to_dict("records"), "latency_us"
        )
        avg_oneway_pivot = self.create_pivot_table(
            avg_oneway_data.to_dict("records"), "latency_us"
        )
        avg_roundtrip_pivot = self.create_pivot_table(
            avg_roundtrip_data.to_dict("records"), "latency_us"
        )
                ...
                [
                    ...
                    # One-way max latency table
                        ...
                        dash_table.DataTable(
                                            ...
                                            data=(
                                                max_oneway_pivot.to_dict(
                                                    "records"
                                                )
                                                if not max_oneway_pivot.empty
                                                else []
                                            ),
                                            columns=(
                                                [
                                                    {"name": i, "id": i}
                                                    for i in max_oneway_pivot.columns
                                                ]
                                                if not max_oneway_pivot.empty
                                                else []
                                            ),
                    ...
                    # Round-trip max latency table
                        ...
                        dash_table.DataTable(
                                            ...
                                            data=(
                                                max_roundtrip_pivot.to_dict(
                                                    "records"
                                                )
                                                if not max_roundtrip_pivot.empty
                                                else []
                                            ),
                                            columns=(
                                                [
                                                    {"name": i, "id": i}
                                                    for i in max_roundtrip_pivot.columns
                                                ]
                                                if not max_roundtrip_pivot.empty
                                                else []
                                            ),
                ...
                [
                    ...
                    # One-way average latency table
                        ...
                        dash_table.DataTable(
                                            ...
                                            data=(
                                                avg_oneway_pivot.to_dict(
                                                    "records"
                                                )
                                                if not avg_oneway_pivot.empty
                                                else []
                                            ),
                                            columns=(
                                                [
                                                    {"name": i, "id": i}
                                                    for i in avg_oneway_pivot.columns
                                                ]
                                                if not avg_oneway_pivot.empty
                                                else []
                                            ),
                    ...
                    # Round-trip average latency table
                        ...
                        dash_table.DataTable(
                                            ...
                                            data=(
                                                avg_roundtrip_pivot.to_dict(
                                                    "records"
                                                )
                                                if not avg_roundtrip_pivot.empty
                                                else []
                                            ),
                                            columns=(
                                                [
                                                    {"name": i, "id": i}
                                                    for i in avg_roundtrip_pivot.columns
                                                ]
                                                if not avg_roundtrip_pivot.empty
                                                else []
                                            ),
        ...
        return html.Div(
            ...
            html.H3("Latency Performance Data"),
                html.P(
                    "Latency measurements organized by message size and mechanism. Lower values indicate better performance.",
                    style={
                        "color": "#8b949e",
                        "margin-bottom": "20px",
                        "fontSize": "0.95rem",
    ...
    def _create_throughput_tables(self, throughput_data: List) -> html.Div:
        ...
        # Filter data for each table type
        oneway_throughput_data = throughput_df_data[
            throughput_df_data["type"] == "One-way"
        ]
        roundtrip_throughput_data = throughput_df_data[
            throughput_df_data["type"] == "Round-trip"
        ]
        
        # Create pivot tables for messages/sec
        oneway_msgs_pivot = self.create_pivot_table(
            oneway_throughput_data.to_dict("records"),
            "msgs_per_sec",
            round_digits=1,
        )
        roundtrip_msgs_pivot = self.create_pivot_table(
            roundtrip_throughput_data.to_dict("records"),
            "msgs_per_sec",
            round_digits=1,
        )
        ...
        roundtrip_bytes_pivot = self.create_pivot_table(
            roundtrip_throughput_mb.to_dict("records"),
            "bytes_per_sec",
            round_digits=1,
        )
                ...
                [
                    ...
                    # One-way messages/sec table
                        ...
                        dash_table.DataTable(
                                            ...
                                            data=(
                                                oneway_msgs_pivot.to_dict(
                                                    "records"
                                                )
                                                if not oneway_msgs_pivot.empty
                                                else []
                                            ),
                                            columns=(
                                                [
                                                    {"name": i, "id": i}
                                                    for i in oneway_msgs_pivot.columns
                                                ]
                                                if not oneway_msgs_pivot.empty
                                                else []
                                            ),
                    ...
                    # Round-trip messages/sec table
                        ...
                        dash_table.DataTable(
                                            ...
                                            data=(
                                                roundtrip_msgs_pivot.to_dict(
                                                    "records"
                                                )
                                                if not roundtrip_msgs_pivot.empty
                                                else []
                                            ),
                                            columns=(
                                                [
                                                    {"name": i, "id": i}
                                                    for i in roundtrip_msgs_pivot.columns
                                                ]
                                                if not roundtrip_msgs_pivot.empty
                                                else []
                                            ),
                ...
                [
                    ...
                    # One-way bytes/sec table
                        ...
                        dash_table.DataTable(
                                            ...
                                            data=(
                                                oneway_bytes_pivot.to_dict(
                                                    "records"
                                                )
                                                if not oneway_bytes_pivot.empty
                                                else []
                                            ),
                                            columns=(
                                                [
                                                    {"name": i, "id": i}
                                                    for i in oneway_bytes_pivot.columns
                                                ]
                                                if not oneway_bytes_pivot.empty
                                                else []
                                            ),
                    ...
                    # Round-trip bytes/sec table
                        ...
                        dash_table.DataTable(
                                            ...
                                            data=(
                                                roundtrip_bytes_pivot.to_dict(
                                                    "records"
                                                )
                                                if not roundtrip_bytes_pivot.empty
                                                else []
                                            ),
                                            columns=(
                                                [
                                                    {"name": i, "id": i}
                                                    for i in roundtrip_bytes_pivot.columns
                                                ]
                                                if not roundtrip_bytes_pivot.empty
                                                else []
                                            ),
        ...
        return html.Div(
            ...
            html.H3("Throughput Performance Data"),
                html.P(
                    (
                        "Throughput measurements organized by message size and mechanism. "
    ...
    def _create_insights_section(self, stats: Dict) -> html.Div:
        ...
        insights = self.generate_performance_insights(
            percentile_df, throughput_df, max_latency_df
        )
        summary_cards = self.create_summary_cards(
            insights, percentile_df, max_latency_df
        )
                                        ...
                                        html.Ul(
                                            [
                                                ...
                                                for rec in insights.get(
                                                    "recommendations",
                                                    [
                                                        "Select data to see recommendations"
                                                    ],
                                                )
                                            ],
    ...
    def _create_comparison_matrices(self, stats: Dict) -> html.Div:
        ...
        p50_comparison_matrix, max_comparison_matrix = (
            self.create_performance_comparison_matrix(
                percentile_df, max_latency_df
            )
        )
        
        # Create performance comparison matrix tables
        comparison_matrix_div = self._create_comparison_matrix_tables(
            p50_comparison_matrix, max_comparison_matrix
        )
    ...
    def _create_summary_charts(
        self, filtered_data: Dict, stats: Dict
    ) -> html.Div:
        ...
        avg_latency_fig, max_latency_fig = self._create_latency_charts(
            streaming_df, mechanism_order, message_size_order
        )
        
        # Create throughput charts  
        msgs_fig, bytes_fig = self._create_throughput_charts(
            throughput_df, mechanism_order, message_size_order
        )
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            [
                                dcc.Graph(
                                    id="avg-latency-chart",
                                    figure=self.apply_dark_theme(
                                        avg_latency_fig
                                    ),
                                )
                            ],
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            [
                                dcc.Graph(
                                    id="throughput-msgs-chart",
                                    figure=self.apply_dark_theme(msgs_fig),
                                )
                            ],
                            className="chart-container",
                        ),
                        html.Div(
                            [
                                dcc.Graph(
                                    id="throughput-bytes-chart",
                                    figure=self.apply_dark_theme(bytes_fig),
                                )
                            ],
                            className="chart-container",
                        ),
    ...
    def _create_latency_charts(
        self,
        streaming_df: pd.DataFrame,
        mechanism_order: List,
        message_size_order: List,
    ):
        ...
        if latency_data:
            ...
            avg_latency_fig = px.bar(
                ...
                labels={
                    "latency_us": "Average Latency (μs)",
                    "message_size": "Message Size (bytes)",
                },
                category_orders={
                    "mechanism": mechanism_order,
                    "message_size": message_size_order,
                },
                color_discrete_map={
                    "One-way": "#10b981",
                    "Round-trip": "#8b5cf6",
            ...
            avg_latency_fig.for_each_annotation(
            ...
            avg_latency_fig.for_each_xaxis(
                lambda x: x.update(
                    showgrid=True, gridcolor="gray", gridwidth=1
                )
            )
            avg_latency_fig.for_each_yaxis(
                lambda y: y.update(
                    showgrid=True, gridcolor="gray", gridwidth=1
                )
            )
            ...
            # Update facet labels to show only the message size value
            max_latency_fig.for_each_annotation(
                lambda a: a.update(text=a.text.split("=")[-1])
            )
            # Add consistent gray gridlines to all axes
            ...
            max_latency_fig.for_each_xaxis(
                lambda x: x.update(
                    showgrid=True, gridcolor="gray", gridwidth=1
                )
            )
            max_latency_fig.for_each_yaxis(
                lambda y: y.update(
                    showgrid=True, gridcolor="gray", gridwidth=1
                )
            )
    ...
    def _create_throughput_charts(
        ...
        if not throughput_df.empty:
            ...
            msgs_fig.for_each_xaxis(
                lambda x: x.update(
                    showgrid=True, gridcolor="gray", gridwidth=1
                )
            )
            msgs_fig.for_each_yaxis(
                lambda y: y.update(
                    showgrid=True, gridcolor="gray", gridwidth=1
                )
            )
            ...
            bytes_fig.for_each_xaxis(
                lambda x: x.update(
                    showgrid=True, gridcolor="gray", gridwidth=1
                )
            )
            bytes_fig.for_each_yaxis(
                lambda y: y.update(
                    showgrid=True, gridcolor="gray", gridwidth=1
                )
            )
        ...
        if latency_data:
            ...
            latency_table_div = html.Div(
                ...
                html.H3("Latency Performance Data"),
                    html.P(
                        (
                            "Latency measurements organized by message size and mechanism. "
    ...
    def _create_comparison_matrix_tables(
        self, p50_matrix, max_matrix
    ) -> html.Div:
        ...
        if not p50_matrix.empty:
            ...
            for col in p50_display.columns:
                if col != "mechanism" and col != "Overall Score":
                    p50_display[col] = p50_display[col].apply(
                        lambda x: (
                            f"+{x}%"
                            if x != "-" and x > 0
                            else f"{x}%" if x != "-" else x
                        )
                    )
            ...
            p50_table = dash_table.DataTable(
                ...
                columns=[
                    {"name": str(i), "id": str(i)} for i in p50_display.columns
                ],
                style_table={
                    "overflowX": "auto",
                    "backgroundColor": "#21262d",
                ...
                style_data_conditional=[
                    {"if": {"row_index": "odd"}, "backgroundColor": "#161b22"},
                    {
                        "if": {"column_id": "mechanism"},
                        "textAlign": "left",
                        "fontWeight": "bold",
                    },
                    {
                        "if": {"column_id": "Overall Score"},
            ...
            matrix_tables.append(
                html.Div(
                    [
                        ...
                        html.P(
                            "Relative P50 latency performance. Lower percentages = better typical performance.",
                            style={
                                "color": "#8b949e",
                                "margin-bottom": "15px",
                                "fontSize": "0.9rem",
        ...
        if not max_matrix.empty:
            ...
            for col in max_display.columns:
                if col != "mechanism" and col != "Overall Score":
                    max_display[col] = max_display[col].apply(
                        lambda x: (
                            f"+{x}%"
                            if x != "-" and x > 0
                            else f"{x}%" if x != "-" else x
            ...
            max_table = dash_table.DataTable(
                ...
                columns=[
                    {"name": str(i), "id": str(i)} for i in max_display.columns
                ],
                style_table={
                    "overflowX": "auto",
                    "backgroundColor": "#21262d",
            ...
            matrix_tables.append(
                html.Div(
                    [
                        ...
                        html.P(
                            "Relative maximum latency performance. Lower percentages = better worst-case performance.",
                            style={
                                "color": "#8b949e",
                                "margin-bottom": "15px",
                                "fontSize": "0.9rem",
    ...
    def _apply_sampling_strategy(
        df: pd.DataFrame, strategy: str, max_points: int
    ) -> pd.DataFrame:
        ...
        if strategy == "uniform":
            ...
            combined = (
                pd.concat([uniform_sample, peaks, valleys])
                .drop_duplicates()
                .sort_index()
            )
            return (
                combined.iloc[:max_points]
            ...
            if len(non_outliers) > 0:
                step_size = max(
                    1, len(non_outliers) // (max_points - len(outliers))
                )
            ...
            combined = (
                pd.concat([sampled_normal, outliers])
                .drop_duplicates()
                .sort_index()
            )
            return (
            ...
            df_copy["variance_window"] = (
                df_copy["one_way_latency_us"]
                .rolling(window=100, min_periods=1)
                .var()
            )
            
            # Sample more from high-variance regions
            high_variance = df_copy[
                df_copy["variance_window"]
                > df_copy["variance_window"].median()
            ]
            low_variance = df_copy[
                df_copy["variance_window"]
                <= df_copy["variance_window"].median()
            ]
            ...
            combined = (
                pd.concat([sampled_high, sampled_low])
                .drop_duplicates()
                .sort_index()
            )
            return (
    ...
    @staticmethod
    def _get_mechanism_colors(
        mechanism: str, latency_type: str, available_mechanisms: List = None
    ) -> str:
        ...
        if mechanism not in base_colors:
            ...
            idx = (
                available_mechanisms.index(mechanism) % len(fallback_colors)
    ...
    def render_summary_tab_cached(self, mechanisms: List, message_sizes: List):
        ...
        return self._render_with_threading(
            self.render_summary_tab, mechanisms, message_sizes, "Summary"
        )
    
    @safe_computation(default_return=html.Div("Error rendering time series"))
    def render_timeseries_tab_cached(
        self, mechanisms: List, message_sizes: List
    ):
    ...
    def _render_with_threading(
        self, render_func, mechanisms, message_sizes, tab_name
    ):
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                future = executor.submit(
                    render_func, mechanisms, message_sizes
                )
                result = future.result(timeout=120)  # 2 minute timeout
            
            computation_time = time.time() - start_time
            logger.info(
                f"{tab_name} analysis completed in {computation_time:.2f}s"
            )
                ...
                [
                    html.Div(
                        [
                            ...
                            html.P(
                                f"An error occurred during {tab_name.lower()} analysis:",
                                style={
                                    "color": "#f0f6fc",
                                    "margin-bottom": "10px",
                                },
                            ...
                            html.P(
                                "Please check your data and filters, then try again.",
                                style={
                                    "color": "#6b7280",
                                    "margin-top": "15px",
                                    "font-style": "italic",
    ...
    def _create_charts_threaded(self, chart_functions):
        ...
        try:
            logger.info(
                f"Creating {len(chart_functions)} charts with threading..."
            )
            start_time = time.time()
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                ...
                futures = {
                    ...
                    for func, name in chart_functions
                }
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        results[chart_name] = future.result(
                            timeout=60
                        )  # 1 minute per chart
                        logger.debug(
                            f"Chart '{chart_name}' created successfully"
                        )
                    except Exception as e:
                        logger.error(
                            f"Error creating chart '{chart_name}': {str(e)}"
                        )
                        results[chart_name] = self._create_error_chart(
                            f"Error in {chart_name}: {str(e)}"
                        )
            
            computation_time = time.time() - start_time
            logger.info(
                f"Chart generation completed in {computation_time:.2f}s"
            )
    ...
    def render_timeseries_tab(self, mechanisms: List, message_sizes: List):
        ...
        controls_section = html.Div(
            [
                html.H3(
                    "Time Series Controls", style={"margin-bottom": "20px"}
                ),
                        ...
                        html.Div(
                            [
                                html.Button(
                                    ...
                                    title=(
                                        "Separate charts with performance zones and percentile bands "
                                ...
                                html.Button(
                                    ...
                                    title=(
                                        "Separate charts with spike detection and distribution overlays "
                                        "for detailed mechanism analysis"
                                    ),
                                ),
                                html.Button(
                                    "Statistical Overview",
                                    id="preset-statistical",
                                    n_clicks=0,
                                    className="preset-btn",
                                    style={
                                        "margin-right": "10px",
            ...
            # Chart Layout & Statistical Controls
                html.Div(
                    [
                        html.Label(
                            "Chart Layout & Statistical Overlays:",
                            style={
                                "margin-bottom": "15px",
                                "display": "block",
                                "font-weight": "bold",
                    ...
                    # Chart Layout
                            ...
                            dcc.RadioItems(
                                ...
                                options=[
                                                        {
                                                            "label": "Separate Charts (by mechanism)",
                                                            "value": "separate",
                                                        },
                                                        {
                                                            "label": "Separate Charts (by message size)",
                                                            "value": "faceted",
                                                        },
                                                    ],
                                                    value="separate",
                                className="chart-layout-radio",
                                            ...
                                            title=(
                                                "Choose how to organize charts: separate charts for each mechanism, "
                                                "or separate charts for each message size"
                                            ),
                                        ),
                        # Statistical Overlays in same column
                            ...
                            dcc.Checklist(
                                ...
                                options=[
                                                        {
                                                            "label": "Percentile Bands (P25-P75)",
                                                            "value": "percentile_bands",
                                                        },
                                                        {
                                                            "label": "Spike Detection (Z-score outliers)",
                                                            "value": "spike_detection",
                                                        },
                                                        {
                                                            "label": "Anomaly Detection (ML pattern analysis)",
                                                            "value": "anomaly_detection",
                                                        },
                                                        {
                                                            "label": "Distribution Overlays",
                                                            "value": "distributions",
                                                        },
                                                        {
                                                            "label": "Rolling P95/P99",
                                                            "value": "rolling_percentiles",
                                                        },
                                                    ],
                                                    value=["percentile_bands"],
                                                    className="filter-checklist",
                                            ...
                                            title=(
                                                "Statistical overlays: confidence bands, spike/anomaly detection, "
                                                "mini histogram overlays, rolling percentile calculations"
                                            ),
                                        ),
                                    ],
                                    style={
                    ...
                    # View Options and Latency Type
                            ...
                            dcc.Checklist(
                                ...
                                options=[
                                            ...
                                            title=(
                                                "View options: sync zoom/pan across charts, show detailed "
                                                "statistics, enable interactive filtering, highlight performance zones"
                                            ),
                                        ),
                        html.Br(),
                                        html.Label(
                            ...
                            dcc.Checklist(
                                ...
                                options=[
                                            ...
                                            title=(
                                                "Select which latency types to display in charts and statistics: "
                                                "one-way (send only) or round-trip (send + receive)"
                                            ),
                                        ),
                                    ],
                                    style={
                ...
                html.Div(
                    ...
                    dcc.Slider(
                        ...
                        value=100,
                                    marks={
                                        i: str(i) for i in [1, 100, 500, 1000]
                                    },
                                    tooltip={
                                        "placement": "bottom",
                                        "always_visible": True,
                    ...
                    dcc.Checklist(
                        ...
                        dcc.RadioItems(
                            ...
                            options=[
                                    ...
                                    title=(
                                        "Y-axis scaling: linear for normal data, logarithmic for wide "
                                        "value ranges or highlighting multiplicative differences"
                                    ),
                                ),
                    html.Br(),
                                html.Label(
                    ...
                    [
                        ...
                        dcc.Dropdown(
                            ...
                            options=[
                                                ...
                                                {
                                                    "label": "Peak-Preserving",
                                                    "value": "peak_preserving",
                                                },
                                                {
                                                    "label": "Outlier-Preserving",
                                                    "value": "outlier_preserving",
                                                },
                                                {
                                    ...
                                    title=(
                                        "Data sampling for large datasets: uniform (even spacing), "
                                        "peak-preserving (maintains peaks), outlier-preserving "
                                        "(keeps extreme values), adaptive (smart density-based)"
                                    ),
                                ),
                            ],
                            style={
            ...
            dcc.Loading(
                ...
                type="dot",
                    children=html.Div(
                        id="ts-stats-panel", style={"margin-bottom": "20px"}
                    ),
                ...
                [
                    html.Div(
                        [
                            ...
                            html.P(
                                (
                                    "Configure your time series analysis settings above and click "
                                    "'Run Analysis' to visualize latency patterns over time."
                                ),
                                style={
                                    "color": "#8b949e",
                                    "font-size": "1.1rem",
                            ...
                            html.Div(
                                [
                                    ...
                                    html.Ul(
                                        [
                                            html.Li(
                                                "Interactive latency trends visualization"
                                            ),
                                            html.Li(
                                                "Statistical overlays and anomaly detection"
                                            ),
                                            html.Li(
                                                "Performance zones and percentile bands"
                                            ),
                                            html.Li(
                                                "Flexible chart layouts and sampling options"
                                            ),
                                            html.Li(
                                                "Real-time statistics and distribution analysis"
                                            ),
        ...
        # Add results container for dynamic updates
        results_container = html.Div(
            id="timeseries-results-container",
            children=html.Div(
                [
                    html.Div(
                        [
                            ...
                            html.P(
                                "Configure your settings above and click 'Run Analysis' to generate time series visualizations.",
                                style={
                                    "color": "#8b949e",
                                    "text-align": "center",
                                    "margin-bottom": "20px",
                            ...
                            html.Ul(
                                [
                                    html.Li("Adjust chart layout and view options"),
                                    html.Li("Select latency types and statistical overlays"),
                                    html.Li("Choose sampling strategy and display points"),
                                    html.Li("Use preset buttons for quick configuration"),
                                ],
                                style={
                                    "color": "#f0f6fc",
                                    "max-width": "500px",
        ...
        return html.Div([controls_section, results_container])

    def render_timeseries_charts_and_stats(
        self,
        mechanisms: List,
        message_sizes: List,
        ...
        """Render ONLY the Time Series charts and stats - no controls."""
                ...
                [
                    html.Div(
                        [
                            ...
                            html.P(
                                "No streaming data available for the selected mechanisms and message sizes.",
                                style={
                                    "color": "#8b949e",
                                    "text-align": "center",
                                    "margin-bottom": "15px",
                            ...
                            html.P(
                                "Try adjusting your filter selections or ensure data contains streaming records.",
                                style={
                                    "color": "#6b7280",
                                    "text-align": "center",
                                    "font-style": "italic",
        ...
        # Generate charts using existing render_timeseries_charts method
        charts_result = self.render_timeseries_charts(
            mechanisms,
            message_sizes,
            moving_avg_window,
            display_points,
        ...
        # Generate stats panel (reusing existing logic from update_stats_panel)
        stats_panel = html.Div()
        if "statistics_panel" in view_options:
            # Generate stats similar to update_stats_panel callback
            if not streaming_df.empty:
                mechanism_tables = []
                    ...
                    for latency_type in latency_types:
                        ...
                        for msg_size in sorted(
                            ...
                            if (
                                ...
                                latency_values = subset_data[latency_col].dropna()
                                stats_data.append(
                                    {
                                        "Message Size": f"{msg_size}B",
                                        "Count": len(latency_values),
                                        "Mean": f"{latency_values.mean():.2f}",
                                        "Std": f"{latency_values.std():.2f}",
                                        "Min": f"{latency_values.min():.2f}",
                                        "P50": f"{latency_values.quantile(0.5):.2f}",
                                        "P95": f"{latency_values.quantile(0.95):.2f}",
                                        "P99": f"{latency_values.quantile(0.99):.2f}",
                                        "Max": f"{latency_values.max():.2f}",
                                    }
                                )
                        ...
                        if stats_data:
                            table = html.Div(
                                [
                                    html.H5(
                                        f"{mechanism} - {type_label} Latency (μs)",
                                        style={
                                            "margin-bottom": "10px",
                                            "color": "#374151",
                                            "font-size": "1rem",
        ...
        # Combine charts and stats panel
            return html.Div([
            html.Div(charts_result, className="charts-container"),
            stats_panel
        ])
    ...
    def render_timeseries_charts(
        ...
        view_options: List = None,
        latency_types: List = None,
        statistical_overlays: List = None,
        y_axis_scale: str = "log",
        y_axis_range: str = "auto",
        sampling_strategy: str = "uniform",
    ):
        """Render the actual time series charts
        with advanced features and multiple layout options."""
        ...
        def apply_sampling_strategy(df, strategy, max_points):
            ...
            if strategy == "uniform":
                ...
                # Get uniform sample of non-outliers
                non_outliers = df[
                    df["one_way_latency_us"] <= outlier_threshold
                ]
                if len(non_outliers) > 0:
                    step_size = max(
                        1, len(non_outliers) // (max_points - len(outliers))
        ...
        if total_original_points > MAX_POINTS_PER_RUN:
            ...
            current_strategy = strategy_info.get(
                sampling_strategy, strategy_info["uniform"]
            )

            performance_note = html.Div(
                [
                    html.P(
                        f"Performance Note: Using {current_strategy['name']} for better interactivity. "
        ...
        def get_mechanism_colors(mechanism, latency_type):
            ...
            available_mechanisms = (
                list(streaming_df["mechanism"].unique())
            ...
            if mechanism not in base_colors:
                ...
                idx = (
                    available_mechanisms.index(mechanism)
                    % len(fallback_colors)
                    if mechanism in available_mechanisms
                    else 0
                )
        ...
        def add_statistical_overlays(
            fig, data, mechanism, color, latency_type="one_way"
        ):
            ...
            type_label = (
                "One-way" if latency_type == "one_way" else "Round-trip"
            )
            
            # Skip if column doesn't exist in data
            if (
                latency_col not in data.columns
                or data[latency_col].isna().all()
            ):
                return
                
            # Add visual overlays for spikes and anomalies
            if (
                "spike_detection" in statistical_overlays
                or "anomaly_detection" in statistical_overlays
            ) and len(data) > 20:
                anomalies = self._detect_anomalies(
                    data, mechanism, latency_type, statistical_overlays
                )
                ...
                for anomaly in anomalies:
                    ...
                    if len(anomaly_data) > 0:
                        if anomaly["type"] == "spikes":
                            # Add spike markers (using magenta to avoid
                            # #conflicts with red mechanism colors)
                            fig.add_trace(
                                go.Scatter(
                                ...
                                marker=dict(
                                    ...
                                    size=10,
                                        symbol="triangle-up",
                                        line=dict(
                                            width=2, color="#ff1744"
                                        ),
                                ),
                                showlegend=True,
                                    hovertemplate="<b>Spike Detected</b><br>%{{x}}<br>%{{y:.2f}} μs<extra></extra>",
                                )
                            )
                        else:  # anomalies or statistical_anomalies
                            # Add anomaly markers (using brown to avoid
                            # conflicts with all mechanism colors)
                            fig.add_trace(
                                go.Scatter(
                                x=anomaly_data.index,
                                y=anomaly_data[latency_col],
                                ...
                                marker=dict(
                                    ...
                                    size=6,
                                        symbol="diamond",
                                        line=dict(
                                            width=1, color="#713f12"
                                        ),  # Dark brown border
                                ),
                                showlegend=True,
                                    hovertemplate="<b>Anomaly Detected</b><br>%{{x}}<br>%{{y:.2f}} μs<extra></extra>",
                                )
                            )
            ...
            if "percentile_bands" in statistical_overlays and len(data) > 50:
                ...
                if window_size > 1:
                    ...
                    fig.add_trace(
                        ...
                        showlegend=True,
                            hoverinfo="skip",
                            hovertemplate="<b>Percentile Band</b><br>P25: %{{y:.2f}} μs<br>Index: %{{x}}<extra></extra>",
                        )
                    )

            if (
                "rolling_percentiles" in statistical_overlays
                and len(data) > 100
            ):
                ...
                if window_size > 10:
                    ...
                    fig.add_trace(
                        ...
                        opacity=0.8,
                        showlegend=True,
                            hovertemplate="<b>P95 Rolling</b><br>Value: %{{y:.2f}} μs<br>Index: %{{x}}<extra></extra>",
                        )
                    )
                    ...
                    fig.add_trace(
                        ...
                        opacity=0.8,
                        showlegend=True,
                            hovertemplate="<b>P99 Rolling</b><br>Value: %{{y:.2f}} μs<br>Index: %{{x}}<extra></extra>",
                        )
                    )
            
            if "distributions" in statistical_overlays and len(data) > 50:
                # Add distribution overlay as a small histogram in upper right
                # corner
                ...
                if len(latency_values) > 20:
                    ...
                    hist_counts, hist_bins = np.histogram(
                        latency_values, bins=20
                    )
                    hist_centers = (hist_bins[:-1] + hist_bins[1:]) / 2
                    
                    # Normalize counts to fit in a small area (scale to 10%
                    # of y-range)
                    y_range = (
                        fig.layout.yaxis.range
                        if fig.layout.yaxis.range
                        else [latency_values.min(), latency_values.max()]
                    )
                    if not y_range or y_range[0] == y_range[1]:
                        y_range = [latency_values.min(), latency_values.max()]
                    
                    y_span = y_range[1] - y_range[0]
                    normalized_counts = (hist_counts / hist_counts.max()) * (
                        y_span * 0.1
                    ) + y_range[1] * 0.85
                    ...
                    fig.add_trace(
                        ...
                        showlegend=True,
                            hovertemplate=(
                                "<b>Distribution</b><br>Latency: %{{y:.2f}} μs<br>"
                                "Frequency: %{{x:.0f}}<extra></extra>"
                            ),
                            xaxis="x2",
                            yaxis="y",
                        )
            ...
            if "anomaly_detection" in statistical_overlays and len(data) > 100:
                ...
                if SKLEARN_AVAILABLE:
                    try:
                        ...
                        anomaly_labels = clf.fit_predict(
                            data[[latency_col]].values
                        )
                        anomaly_data = data[anomaly_labels == -1]
                        ...
                        anomaly_threshold = q75 + 2.0 * iqr
                        anomaly_data = data[
                            data[latency_col] > anomaly_threshold
                        ]
        ...
        if chart_layout == "faceted":
            ...
            message_sizes_available = sorted(
                streaming_df["message_size"].unique()
            )
            
            for msg_size in message_sizes_available:
                size_data = streaming_df[
                    streaming_df["message_size"] == msg_size
                ]
                ...
                if len(size_data) > 0:
                    ...
                    for i, mechanism in enumerate(
                        size_data["mechanism"].unique()
                    ):
                        symbol = symbols[i % len(symbols)]

                        mechanism_df = size_data[
                            size_data["mechanism"] == mechanism
                        ]
                        run_data = self._apply_sampling_strategy(
                            mechanism_df, sampling_strategy, MAX_POINTS_PER_RUN
                        )
                        run_data = run_data.reset_index(drop=True)
                        
                        # Add statistical overlays for each selected latency
                        # type
                        for latency_type in latency_types:
                            latency_col = (
                                "one_way_latency_us"
                            ...
                            if (
                                latency_col in run_data.columns
                                and not run_data[latency_col].isna().all()
                            ):
                                overlay_color = self._get_mechanism_colors(
                                    mechanism,
                                    latency_type,
                                    (
                                        list(
                        ...
                        for latency_type in latency_types:
                            ...
                            trace_color = self._get_mechanism_colors(
                                mechanism,
                                latency_type,
                                (
                                    list(streaming_df["mechanism"].unique())
                            ...
                            # Skip if column doesn't exist in data
                            if (
                                latency_col not in run_data.columns
                                or run_data[latency_col].isna().all()
                            ):
                                continue
                            ...
                            if (
                                "moving_avg" in display_options
                                and len(run_data) > moving_avg_window
                            ):
                                ma_col = f"{latency_type}_ma"
                                run_data[ma_col] = (
                                    run_data[latency_col]
                                    .rolling(
                    ...
                    if "perf_zones" in view_options and len(size_data) > 0:
                        ...
                        for latency_type in latency_types:
                            ...
                            if (
                                latency_col in size_data.columns
                                and not size_data[latency_col].isna().all()
                            ):
                                overall_p50 = size_data[latency_col].quantile(
                                    0.5
                                )
                                overall_p95 = size_data[latency_col].quantile(
                                    0.95
                                )

                                # Use distinct colors for performance zones of
                                # different latency types
                                zone_color_p50 = (
                                    "#10b981"
                                    if latency_type == "one_way"
                                    else "#059669"
                                ...
                                fig.add_hline(
                                    y=overall_p50,
                                    line_dash="dash",
                                    line_color=zone_color_p50,
                                    annotation_text=f"Good P50 ({type_label})",
                                    annotation_position="bottom right",
                                )
                                fig.add_hline(
                                    y=overall_p95,
                                    line_dash="dash",
                                    line_color=zone_color_p95,
                                    annotation_text=f"Warning P95 ({type_label})",
                                    annotation_position="bottom right",
                                )

                    # Update chart layout for this message
                    #  size (moved inside chart creation block)
                    fig.update_layout(
                        ...
                        yaxis_autorange=(
                            True if y_axis_range == "auto" else False
                        ),
                        showlegend=True,
                        font=dict(size=12),
                        height=500,
                    ...
                    charts.append(
                        html.Div(
                            [
                                dcc.Graph(
                                    figure=self.apply_dark_theme(fig),
                                    id=f"faceted-chart-{msg_size}",
                                )
                            ],
                            className="chart-container",
            ...
            for i, mechanism in enumerate(streaming_df["mechanism"].unique()):
                ...
                mechanism_df = streaming_df[
                    streaming_df["mechanism"] == mechanism
                ]
                
                if len(mechanism_df) > 0:
                    # Get unique message sizes for
                    #  this mechanism to create separate traces
                    mechanism_message_sizes = sorted(
                        mechanism_df["message_size"].unique()
                    )
                    ...
                    for size_idx, msg_size in enumerate(
                        mechanism_message_sizes
                    ):
                        # Filter data for this specific message size
                        size_df = mechanism_df[
                            mechanism_df["message_size"] == msg_size
                        ]
                        ...
                        run_data = self._apply_sampling_strategy(
                            size_df,
                            sampling_strategy,
                            MAX_POINTS_PER_RUN // len(mechanism_message_sizes),
                        )
                        run_data = run_data.reset_index(drop=True)
                        ...
                        # Detect anomalies for each selected
                        #  latency type for this message size
                        for latency_type in latency_types:
                            latency_col = (
                                "one_way_latency_us"
                                if latency_type == "one_way"
                        ...
                        # Generate traces for each selected
                        #  latency type and message size combination
                        for latency_type in latency_types:
                            latency_col = (
                                "one_way_latency_us"
                                if latency_type == "one_way"
                            ...
                            # Create trace name with both
                            #  message size and latency type
                    ...
                    # Add statistical overlays per message
                    #  size for better granularity
                    for size_idx, msg_size in enumerate(
                        mechanism_message_sizes
                    ):
                        size_df = mechanism_df[
                        ...
                        run_data_overlay = self._apply_sampling_strategy(
                            size_df,
                            sampling_strategy,
                            MAX_POINTS_PER_RUN // len(mechanism_message_sizes),
                        )
                        run_data_overlay = run_data_overlay.reset_index(
                            drop=True
                        )
                        ...
                        for latency_type in latency_types:
                            ...
                            if (
                                latency_col in run_data_overlay.columns
                                and not run_data_overlay[latency_col]
                                .isna()
                                .all()
                            ):
                                # Add statistical overlays
                                # with message size context
                                add_statistical_overlays(
                                    fig,
                                    run_data_overlay,
                                    f"{mechanism} {msg_size}B",
                                    size_color,
                                    latency_type,
                                )
                ...
                # Add performance zones if enabled - show
                # overall mechanism zones for reference
                if "perf_zones" in view_options and len(mechanism_df) > 0:
                    # Add overall mechanism performance
                    # zones for each selected latency type
                    for latency_type in latency_types:
                        latency_col = (
                            "one_way_latency_us"
                            if latency_type == "one_way"
                        ...
                        if (
                            latency_col in mechanism_df.columns
                            and not mechanism_df[latency_col].isna().all()
                        ):
                            overall_p50 = mechanism_df[latency_col].quantile(
                                0.5
                            )
                            overall_p95 = mechanism_df[latency_col].quantile(
                                0.95
                            )

                            # Use distinct colors for performance
                            # zones of different latency types
                            zone_color_p50 = (
                                "#10b981"
                                if latency_type == "one_way"
                                else "#059669"
                            ...
                            fig.add_hline(
                                ...
                                line_dash="dash",
                                line_color=zone_color_p50,
                                annotation_text=f"Overall P50 ({type_label})",
                                annotation_position="bottom right",
                            )
                            fig.add_hline(
                                y=overall_p95,
                                line_dash="dash",
                                line_color=zone_color_p95,
                                annotation_text=f"Overall P95 ({type_label})",
                                annotation_position="bottom right",
                            )

                # Update chart layout for this mechanism
                # (moved outside performance zones block)
                fig.update_layout(
        ...
        return html.Div([performance_note, *charts, stats_panel])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Rusty-Comms Benchmark Results Dashboard"
    )
    parser.add_argument(
        "--dir",
        required=True,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable debug mode (can cause reloading issues)",
    )
    ...
    # Check if data was loaded - but don't exit for empty directories
    summary_count = len(data_store["summary_data"])
    streaming_count = len(data_store["streaming_data"])
    
    if summary_count == 0 and streaming_count == 0:
        logger.info(
            "No benchmark data found - starting with empty dashboard. "
            "Use 'Refresh Data' button to load data after adding files."
        )
    else:
        logger.info(
            "Loaded %d summary records and %d streaming records", 
            summary_count, streaming_count
        )
