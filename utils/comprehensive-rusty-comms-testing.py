...
def set_selinux_permissive() -> bool:
    ...
    try:
        result = subprocess.run(
            ["setenforce", "0"], capture_output=True, text=True, timeout=5
        )
...
def check_and_update_qm_binary() -> bool:
    ...
    if result.returncode != 0 or result.stdout.strip() != "true":
        print(
            f"WARNING: QM container '{QM_CONTAINER}' not running, skipping H2QM tests"
        )
    ...
    # Compare times (use integer seconds since QM stat
    # doesn't report sub-second precision)
    if int(qm_mtime) >= int(host_mtime):
        print("QM binary is up-to-date")
        return True

    # Need to update
    if qm_mtime == 0:
        print("QM binary not found, copying...")
    else:
    ...
    print("QM binary updated successfully")
    return True
...
class TestConfig:
    ...
    def output_name(self) -> str:
        return (
            f"{self.mode.value}_{self.mechanism}"
            f"_{self.size}_{self.test_type}"
            "_summary.json"
        )
...
def run_command(
    ...
    Args:
        ...
        stream_file: Deprecated - streaming CSV is now
            handled via --streaming-output-csv flag
    """
        ...
        if stream:
            ...
            try:
                ...
                return subprocess.CompletedProcess(
                    cmd, 124, "".join(stdout_lines), "Timeout"
                )
...
def wait_for_tcp_listener_ss(
    ss_cmd: List[str], port: int, timeout: float, label: str
) -> bool:
    """Wait until a TCP listener is visible via `ss -ltn`.

    This is intentionally non-destructive: unlike active connect probes, it does
    not create a test connection that could consume a single accepted session.
    """
    deadline = time.time() + timeout
    ...
    if ss_unavailable:
        # Fall back to best-effort behavior if `ss` is unavailable in the target
        # environment. Keep this explicit so users know readiness confidence is lower.
        print(
            f"  WARNING: Could not use 'ss' to verify {label} readiness; "
            "falling back to short delay"
        )
        time.sleep(2)
        return True

    print(f"  FAILED: Timed out waiting for {label} listener on TCP port {port}")
    return False


def wait_for_pmq_queues(base_name: str, timeout: float, label: str) -> bool:
    """Wait until both PMQ request/response queue files exist."""
    # PMQ transports create two queues from the configured base name:
    #   <base>_req and <base>_resp
    # /dev/mqueue entries do not include a leading slash.
    normalized = base_name.lstrip("/")
    req_path = Path(f"/dev/mqueue/{normalized}_req")
    resp_path = Path(f"/dev/mqueue/{normalized}_resp")
    deadline = time.time() + timeout
    ...
    print(
        f"  FAILED: Timed out waiting for {label}: "
        f"{req_path.name} and {resp_path.name}"
    )
    return False
...
def run_standalone_test(config: TestConfig, stream_file: Optional[Path] = None) -> bool:
    ...
    if config.mode == Mode.STANDALONE_BLOCKING:
        cmd.append("--blocking")
        # Use shm-direct for SHM in blocking mode
        # (supports bidirectional via dual slots)
    ...
    if result.returncode != 0:
        print(
            f"  FAILED: {result.stderr[:200] if result.stderr else 'No error output'}"
        )
        return False
...
def run_h2qm_test(
    config: TestConfig, stream_file: Optional[Path] = None
) -> Optional[bool]:
    ...
    if config.mechanism == "tcp":
        ...
        readiness_check = lambda: wait_for_tcp_listener_ss(  # noqa: E731
            ["podman", "exec", QM_CONTAINER, "ss", "-ltn"],
            8080,
            15,
            "QM TCP server",
        )
        ...
        # Server (QM) sees: /var/tmp/ipc_test/...
        # Sender (host) sees: /var/qm/tmp/ipc_test/...
        ...
        server_extra = [
            "--socket-path",
            f"/var/tmp/ipc_test/{socket_name}",
        ]
        sender_extra = [
            "--socket-path",
            str(socket_path_host),
        ]
        readiness_check = lambda: wait_for_path(  # noqa: E731
            socket_path_host,
            15,
            "QM UDS socket",
        )
    elif config.mechanism == "shm":
        # Uses bind-mounted /dev/shm with default name
        ...
        # SELinux qm_t blocks ALL POSIX message queue
        # operations. Skip when SELinux is Enforcing.
        if get_selinux_mode() == "Enforcing":
            print(
                "  SKIPPED: SELinux Enforcing blocks"
                " PMQ inside QM (qm_t denies mqueue"
                " access).\n"
                "    To run this test:\n"
                "      - With this script: re-run "
                "with --allow-selinux-permissive\n"
                "      - Manually: setenforce 0  "
                "(restore with setenforce 1)"
            )
            return None
        mq_name = f"ipc_h2qm_mq_{config.size}"
        server_extra = ["--message-queue-name", mq_name]
        sender_extra = ["--message-queue-name", mq_name]
        readiness_check = lambda: wait_for_pmq_queues(  # noqa: E731
            mq_name,
            20,
            "QM PMQ queues",
        )
    ...
    if result.returncode != 0:
        ...
        print(f"  FAILED to start server: {stderr_snip}")
        return False
...
def cleanup_c2c_containers():
    ...
    for container in [C2C_SERVER_CONTAINER, C2C_SENDER_CONTAINER]:
        subprocess.run(
            ["podman", "rm", "-f", container], capture_output=True, timeout=30
        )
...
def cleanup_h2nqm_container():
    ...
    subprocess.run(
        ["podman", "rm", "-f", H2NQM_SERVER_CONTAINER], capture_output=True, timeout=30
    )
...
def run_h2nqm_test(config: TestConfig, stream_file: Optional[Path] = None) -> bool:
    ...
    if config.mechanism == "tcp":
        if not wait_for_tcp_listener_ss(["ss", "-ltn"], 8080, 15, "H2NQM TCP server"):
            cleanup_h2nqm_container()
            return False
    elif config.mechanism == "uds":
        if not wait_for_path(
            H2NQM_SHARED_DIR / f"h2nqm_{config.size}.sock", 15, "H2NQM UDS socket"
        ):
            cleanup_h2nqm_container()
            return False
    elif config.mechanism == "shm":
        if not wait_for_path(
            Path("/dev/shm/ipc_benchmark_shm"), 20, "H2NQM SHM segment"
        ):
            cleanup_h2nqm_container()
            return False
    elif config.mechanism == "pmq":
        if not wait_for_pmq_queues(f"h2nqm_mq_{config.size}", 20, "H2NQM PMQ queues"):
            cleanup_h2nqm_container()
            return False
...
def setup_c2c_shared_resources(
    mechanism: str,
    size: int,
) -> Dict[str, str]:
    """Set up shared resources for C2C testing.

    Returns paths/names for server and sender.
    """
...
def run_c2c_test(config: TestConfig, stream_file: Optional[Path] = None) -> bool:
    ...
    if result.returncode != 0:
        stderr_snip = result.stderr[:200] if result.stderr else "No output"
        print(f"  FAILED to start server container: " f"{stderr_snip}")
        cleanup_c2c_containers()
        return False

    # Wait for server readiness via mechanism-specific artifacts/endpoints.
    if config.mechanism == "tcp":
        if not wait_for_tcp_listener_ss(["ss", "-ltn"], 8080, 15, "C2C TCP server"):
            cleanup_c2c_containers()
            return False
    elif config.mechanism == "uds":
        if not wait_for_path(
            C2C_SHARED_DIR / f"c2c_{config.size}.sock", 15, "C2C UDS socket"
        ):
            cleanup_c2c_containers()
            return False
    elif config.mechanism == "shm":
        ...
        if not wait_for_pmq_queues(f"c2c_mq_{config.size}", 20, "C2C PMQ queues"):
            cleanup_c2c_containers()
            return False
    ...
    if stream_file:
        sender_cmd.extend(
            ["--streaming-output-csv", f"/output/{config.streaming_name}"]
        )

    sender_cmd.append("--round-trip")

    print("  Running sender container...")
    result = run_command(sender_cmd, timeout=600, stream=STREAM_OUTPUT)
...
def run_qm_c2c_test(
    ...
    if config.mechanism == "tcp":
        ...
        # SELinux qm_t blocks ALL POSIX message queue operations
        # (both creating and opening). Skip when SELinux is Enforcing.
        if get_selinux_mode() == "Enforcing":
            print(
                "  SKIPPED: SELinux Enforcing blocks PMQ inside "
                "QM (qm_t denies mqueue access).\n"
                "    To run this test:\n"
                "      - With this script: re-run with "
                "--allow-selinux-permissive\n"
                "      - Manually: setenforce 0  (restore with "
                "setenforce 1)"
            )
            return None
    ...
    # Justification: Wait for the server process to create its
    # resources (listener/segment/queues) before the sender connects.
    time.sleep(3)
        ...
        if stream_file and qm_streaming_path:
            copy_result = subprocess.run(
                [
                    "podman",
                    "cp",
                    f"{QM_CONTAINER}:{qm_streaming_path}",
                    str(stream_file),
                ],
                capture_output=True,
                timeout=30,
...
def run_test(config: TestConfig, stream_file: Optional[Path] = None) -> Optional[bool]:
    ...
    """
    print(
        f"[{config.mode.value}] {config.mechanism} "
        f"size={config.size} type={config.test_type}",
        flush=True,
    )
...
def generate_csv_summary():
    ...
    for jf in json_files:
        try:
            ...
            # JSON: results[0].test_config,
            #       results[0].one_way_results, etc.
...
def should_stream_test(
    config: TestConfig,
    stream_sizes: Optional[List[int]],
    stream_iter_only: bool,
    stream_dur_only: bool,
) -> bool:
...
""",
    ...
    mode_group.add_argument(
        "--h2qm-only", action="store_true", help="Only run Host-to-QM tests"
    )
    mode_group.add_argument(
        "--h2nqm-only",
        action="store_true",
    ...
    mode_group.add_argument(
        "--blocking-only", action="store_true", help="Only run blocking mode tests"
    )
    mode_group.add_argument(
        "--async-only", action="store_true", help="Only run async mode tests"
    )
    mode_group.add_argument(
        "--mechanisms",
        type=str,
        default=None,
        help=(
            "Comma-separated list of mechanisms to run "
            "(e.g., 'pmq', 'tcp,uds'). "
            "Choices: uds, tcp, shm, pmq. Default: all"
        ),
    )
    mode_group.add_argument(
        "--allow-selinux-permissive",
    ...
    stream_group.add_argument(
        "--iter-only", action="store_true", help="Stream only iteration-based tests"
    )
    stream_group.add_argument(
        "--dur-only", action="store_true", help="Stream only duration-based tests"
    )
    ...
    # Parse mechanism filter
    all_mechanisms = ["uds", "tcp", "shm", "pmq"]
    if args.mechanisms:
        requested = [m.strip().lower() for m in args.mechanisms.split(",")]
        invalid = [m for m in requested if m not in all_mechanisms]
        if invalid:
            print(
                f"ERROR: Unknown mechanism(s): {', '.join(invalid)}. "
                f"Valid choices: {', '.join(all_mechanisms)}"
            )
            sys.exit(1)
        selected_mechanisms = requested
    else:
        selected_mechanisms = all_mechanisms
    ...
    # Standalone modes are always included via
    # get_modes_for_mechanism() unless an --*-only flag
    # restricts the run.
    include_h2qm = not (
    ...
    if args.mechanisms:
        print(f"Mechanisms: {', '.join(selected_mechanisms)}")
    ...
    results: Dict[str, int] = {"passed": 0, "failed": 0, "skipped": 0}
    failed_tests = []
    ...
    for test_type in test_types:
        ...
        for mechanism in mechanisms:
            ...
            if args.blocking_only:
                modes = [
                    m for m in modes if "blocking" in m.value or "shm_direct" in m.value
                ]
            ...
            for mode in modes:
                ...
                for size in sizes:
                    config = TestConfig(
                        mechanism=mechanism, mode=mode, size=size, test_type=test_type
                    )
                    ...
                    stream_file = (
                        OUTPUT_DIR / config.streaming_name if use_stream else None
                    )
